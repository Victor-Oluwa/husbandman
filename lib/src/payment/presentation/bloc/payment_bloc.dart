...
class PaymentBloc extends Bloc<PaymentEvent, PaymentState> {
  PaymentBloc(
      ...
      AddNewCardFundingHistory addNewCardFundingHistory,
      UpdateCardFundingHistory updateCardFundingHistory,
      FetchCardFundingHistory fetchCardFundingHistory,)
      : _addNewCard = addNewCard,
        ...
        _addNewCardFundingHistory = addNewCardFundingHistory,
        _updateCardFundingHistory = updateCardFundingHistory,
        _fetchCardFundingHistory = fetchCardFundingHistory,
    ...
    on<AddNewCardFundingHistoryEvent>(_addNewCardFundingHistoryHandler);
    on<UpdateCardFundingHistoryEvent>(_updateCardFundingHistoryHandler);
    on<FetchCardFundingHistoryEvent>(_fetchCardFundingHistoryHandler);
  }
  ...
  final AddNewCardFundingHistory _addNewCardFundingHistory;
  final UpdateCardFundingHistory _updateCardFundingHistory;
  final FetchCardFundingHistory _fetchCardFundingHistory;
  ...
  Future<void> _addNewCardFundingHistoryHandler(
    AddNewCardFundingHistoryEvent event,
    Emitter<PaymentState> emit,
  ) async {
    final result = await _addNewCardFundingHistory(event.history);

    result.fold(
      (l) => emit(PaymentHistoryError(l.errorMessage)),
      (r) => emit(
        AddedNewCardFundingHistory(r),
      ),
    );
  }

  Future<void> _updateCardFundingHistoryHandler(
    UpdateCardFundingHistoryEvent event,
    Emitter<PaymentState> emit,
  ) async {
    final result = await _updateCardFundingHistory(
      UpdateCardFundingHistoryParams(
    ...
    result.fold(
      (l) => emit(PaymentHistoryError(l.errorMessage)),
      (r) => emit(
        UpdatedCardFundingHistory(r),
      ),
    );
  }

  Future<void> _fetchCardFundingHistoryHandler(
    FetchCardFundingHistoryEvent event,
    Emitter<PaymentState> emit,
  ) async {
    final result = await _fetchCardFundingHistory();

    result.fold(
      (l) => emit(PaymentHistoryError(l.errorMessage)),
      FetchedCardFundingHistory.new,
    );
  }
}
